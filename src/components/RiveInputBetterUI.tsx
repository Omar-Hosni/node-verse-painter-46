...
export const RiveInput: React.FC<{ nodeType: string }> = ({ nodeType }) => {
  ...
  const { hasImageInputConnections, setNodes, setEdges, initializeServices } = useWorkflowStore();
  ...
  const projectId = location.pathname.split("/").pop();
    ...
    // Check if this ControlNet node has image input connections
    const hasImageConnections = selectedNode ? hasImageInputConnections(selectedNode.id) : false;
    const preprocessedImage = selectedNode?.data?.preprocessedImage;
    const showPreprocessedImage = hasImageConnections && preprocessedImage;
  ...
  return (
    <div>
      {/* Rive Preview as a property with exact same anatomy - no label */}
      <div className="flex items-center">
        <div className="flex-1 flex gap-1.5 justify-center items-center" style={{ minHeight: '235px', height: '235px' }}>
          <div className="w-[235px] h-[235px] overflow-hidden" style={{ borderRadius: '19px', border: '1.5px solid #1d1d1d' }}>
            <RiveComponent
