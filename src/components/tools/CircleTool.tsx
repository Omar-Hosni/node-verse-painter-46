...
export function CircleTool() {
    ...
    // Helper function to create a circle node
    function createCircleNode(position: XYPosition, width: number, height: number) {
        const order = getHighestOrder(getNodes()) + 1;
        const nodeId = `circle-node-${Date.now()}`;

        const newNode = {
            id: nodeId,
            type: 'circle-node' as const,
            position,
            zIndex: order,
            draggable: true,
            data: {
    ...
    function handlePointerUp(e: PointerEvent) {
        ...
        if (isDragging && end) {
            ...
            // Only create circle if it has meaningful size (at least 10x10 pixels)
            if (dimension.width >= 10 && dimension.height >= 10) {
                createCircleNode(position, dimension.width, dimension.height);
            }
        } else {
            // Handle click to create 400x400 circle
            const rect = (e.target as HTMLElement).getBoundingClientRect();
            const clickScreen = { x: start.x + rect.left, y: start.y + rect.top };
            const clickPosition = screenToFlowPosition(clickScreen);

            // Center the 400x400 circle on the click position
            const centeredPosition = {
                x: clickPosition.x - 200, // Half of 400
                y: clickPosition.y - 200  // Half of 400
            };

            createCircleNode(centeredPosition, 400, 400);
        }
        ...
        // Return to select tool after creating circle
