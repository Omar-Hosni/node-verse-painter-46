...
export function FrameTool() {
    ...
    // Helper function to create a frame node
    function createFrameNode(position: XYPosition, width: number, height: number) {
        const order = getHighestOrder(getNodes()) + 1;
        const nodeId = `frame-node-${Date.now()}`;

        const newNode = {
            id: nodeId,
            type: 'frame-node' as const,
            position,
            zIndex: order,
            draggable: true,
            data: {
    ...
    function handlePointerUp(e: PointerEvent) {
        ...
        if (isDragging && end) {
            ...
            // Only create frame if it has meaningful size (at least 10x10 pixels)
            if (dimension.width >= 10 && dimension.height >= 10) {
                createFrameNode(position, dimension.width, dimension.height);
            }
        } else {
            // Handle click to create 1024x1024 frame
            const rect = (e.target as HTMLElement).getBoundingClientRect();
            const clickScreen = { x: start.x + rect.left, y: start.y + rect.top };
            const clickPosition = screenToFlowPosition(clickScreen);

            // Center the 1024x1024 frame on the click position
            const centeredPosition = {
                x: clickPosition.x - 512, // Half of 1024
                y: clickPosition.y - 512  // Half of 1024
            };

            createFrameNode(centeredPosition, 1024, 1024);
        }
        ...
        // Return to select tool after creating frame
