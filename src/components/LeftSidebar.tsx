...
export const LeftSidebar = () => {
  ...
  return (
    <div className="w-16 lg:w-64 h-full bg-sidebar border-r border-gray-700 flex flex-col overflow-hidden transition-all duration-200">
      ...
      {/* Use ScrollArea to make content scrollable */}
            ...
            <div className="space-y-8">
              {insertCategories.map((category) => (
                <div key={category.name} className="mb-8">
                  <div className="node-category-title flex items-center mb-3">
                    <category.icon className="h-4 w-4 mr-2" />
                    <span className="font-medium text-gray-300">{category.name}</span>
                  </div>
                  <div className="flex flex-col space-y-2">
                    {/* Display options in a flex-row layout */}
                    <div className="flex flex-row flex-wrap gap-3">
                      {category.options.map((option) => (
                        <div 
                          key={option.type}
                          onClick={() => handleAddNode(option.type)}
                          className="node-option p-3 rounded-lg bg-gray-800/50 hover:bg-gray-700/70 border border-gray-700/50 hover:border-blue-500/50 transition-all duration-200 cursor-pointer w-[90px] h-[90px]"
                        >
                          <div className="flex flex-col items-center justify-center h-full">
                            <div className="node-option-icon mb-2 bg-blue-500/20 p-2 rounded-md">
                              <option.icon className="h-6 w-6 text-blue-400" />
                            </div>
                            <span className="text-xs text-center font-medium text-white tracking-tight">{option.label}</span>
                          </div>
                        </div>
                      ))}
                    </div>
